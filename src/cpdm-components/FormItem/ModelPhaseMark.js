...
class ModelPhaseMark extends React.PureComponent {
  componentDidMount() {
    const {
      dispatch,
      options,
      dictEntries: { ModelPhaseMark: modelPhaseMarks },
    } = this.props;
    if (!options && !modelPhaseMarks) dispatch({ type: 'common/modelPhaseMarks' });
  }
  ...
  render() {
    ...
    return (
      <Select
        ...
        style={{ width: 65 }}
      >
        {(options || modelPhaseMarks || []).map(mark => (
