  ...
  useEffect(() => {
    (async () => {
      ...
      if (res && Array.isArray(res)) {
        ...
        if (onRowClick && res.length) onRowClickFn(res[0]);
      } else {
        setState({ ...state, dataSource: [], loading: false });
      }
    })();
  }, [cardType, data.id, dataType, id, objectType, onRowClick, onRowClickFn, state.refreshKey]);
    ...
    <>
      <Card size="small" className="noPaddingCard">
        <Table
          ...
          style={{ marginTop: 1 }}
          size="small"
          columns={column || columns.map(item => ({ ...item, ellipsis: true }))}
          ...
          onRow={record => {
            return {
              onClick: () => onRowClickFn(record), // 点击行
            };
          }}
          rowClassName={r => (r.id === selected ? 'selectable-row-selected' : undefined)}
          toolBarRender={() =>
            toolBarRender || [
              <Button
                size="small"
