...
class DataSearchModal extends PureComponent {
  ...
  render() {
    const lifecycleStatus = [
      { name: '正在工作', value: 'INWORK' },
      { name: '正在审阅', value: 'UNDERREVIEW' },
      { name: '返工', value: 'REWORK' },
      { name: '已发布', value: 'RELEASED' },
    ];
    const { BASE_PATH } = process.env;
    const { title, searching, visible, checkType, confirmLoading, children, style } = this.props;
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'state',
        width: '9%',
        filters: (lifecycleStatus || []).map(item => ({
          text: item.name,
          value: item.value,
        })),
        render(text) {
