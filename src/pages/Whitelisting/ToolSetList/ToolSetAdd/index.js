...
const ToolSetAdd = props => {
  const [rowKeys, setRowKeys] = useState([]);
  const [row, setRow] = useState('');
  const [form] = Form.useForm();
  const formref = React.createRef();
  const searchAddData = useRef();
  const searchAddTreeData = useRef();
  ...
  const onSelect = (selected, e) => {
    ...
    searchAddTreeData.current = classificationId;
    getList({
      ...searchAddData.current,
      ...searchAddTreeData.current,
      ...searchMajor.current,
      ...searchSys.current,
  ...
  const onReset = () => {
    ...
    searchAddData.current = undefined;
    searchAddTreeData.current = undefined;
    searchMajor.current = undefined;
  ...
  return (
    <PageHeaderWrapper title="工具集入库" extra={extra} breadcrumb={false}>
